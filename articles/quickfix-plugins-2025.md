...
quicker.nvimはバッファで行を消しただけでは、表面上変化しているだけです。
後述するnvim-bqfでpreviewを出している場合はズレます。
保存すればpreviewも同期するので、候補を削った際は保存すると良いでしょう。
...
`nvim-bqf`を併用してquickfixを編集する場合、keymapが邪魔になる場合があるため、`nvim-bqf`の設定で不要なマッピングを削除するか、他のキーに移動しましょう。
